...
introduce an abstract type \tc{Scope} and a function \tc{withRefreshed}.

The first approach prevents the user from inserting a name into a scope if that
name is in the scope already. In such case, a fresh name has to be inserted
instead.\improvement{Add member in the code snippet} The types are a little
...
glue with the Liquid Haskell world.\change{Arnaud's at this point for now}
