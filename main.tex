...
\item Every traversed binder must be renamed if it's already a member of the
      scope set.\unsure{Is this true? What happens if you don't rename and
      simply shadow? Answer: then it can capture free variables in the range
        of the substitution.}
...
\unsureF{Not all of it, they missed the last requirement. I miss a summary at the
end of the previous section making explicit what the foil achieved.}
...
We would
like to argue that refinement types, like in Liquid Haskell, can server to
reduce ingenuity in program design. The code presented in
...
substitute :: Set Int -> Subst Exp -> Exp -> Exp
\end{verbatim}
\change{I'm running out of steam, but we want to remove the example from the
  substitution from the primer part. We probably want to move the primer to the
...
\tc{domain} appears:\unsure{Something we should be part of the discussion
  ...
  these axioms with an concrete substitution type? Answer: you copy the assumptions
  to address whatever substitution representation is needed.}
...
naked terms.\unsure{I'm considering adding a comment on the fact that we can use
  ...
  the static checks.}
\unsureF{I'm also missing a summary here of what our annotations have achieved. It
should be noted in particular, that this doesn't address requirement (2) which
motivates the discussion in the next subsection.}

\subsection{A hybrid approach}
\label{ensuring-the-scope-set-is-checked}

\change{I don't have enough time today but my intent is to change the narrative
  of this section to show that you can, and it's beneficial mix the abstract
  type style and the refinement type style. The game plan is to say that a
  property like “a scope is only ever extended with a fresh variable” aren't
  natural with refinement types. But they are incredibly easy with sealed
  types.} Checking that the scope set is actually used, and therefore that
binders to rename are not forgotten, admits at least two
approaches.\improvement{I don't think we need to care about the fact that
  there's more than one way, be assertive and commit to a choice} In both cases
we will prevent the user from making a recursive call to \tc{substitute} without
checking first if the binder needs renaming.
...
The source code of this section can be found in \sourcefile{Unif.hs}.
