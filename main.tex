...
\title{Refinement-Types Driven Development}
...
of sets in the SMT solver. While that \tc{member} verifies this signature isn't
checked, Liquid Haskell will assume the property in the return refinement type
...
until pretty much the very end. This doesn't cause any problem as, due to refinement types'
...
loop and any attempt at using \tc{bad} would diverge. But \tc{bad} is
...
So to support this study, we implemented the theory of finite maps for Liquid
...
allows for a direct comparison between type methods and refinement methods. We
...
Section~\ref{the-rapier-with-stronger-types}). This is in large part due to the
fact that because refinement types can enforce invariants without the need for
abstract types, and such an open interface can be extended in user-space.
Contrast with the abstract-type approach where you have to design, upfront, a
set of invariant-preserving operation sufficient to express downstream programs.
...
author may need to mobilize comparable effort for refinement type as she would
...
non-trivial number of bugs and user-experience defects while conducting our
...
That is sometimes faster than it takes to compile a modules with the GHC compiler.
...
SMT solver (54 seconds checking unification, 4 seconds checking \tc{Subst2.hs},
...
$$\mathit{skolemize} :: \mathit{Scope}\ s_1 \ldots s_n \rightarrow \mathit{Formula}\ f_1 \ldots f_j \rightarrow \mathit{State} t_1 \ldots t_k (\mathit{Scope}\ e_1 \ldots e_l) (\mathit{Formula}\ o_1 \ldots o_m)]$$
...
The tooling isn't ready for widespread use. Yet it's plausible that a decently
close future, we have access to SMT- and refinement-types to assist us in
our programming.
...
in situation where we can manage to use current typecheckers today. As a closing
