...
They handle more cases in the reference source code, but these cases aren't essential to our
discussion.
...
input formula. The finite map \tc{m} in the monadic state contains the variables
that have been introduced with existential quantification together with their
scopes, that is, the universally quantified variables in scope at each existential
binder.
...
We also really need to put the map \tc{,} in the monadic state, as we don't
...
The main conjuncts of the postconditon are \tc{consistent\-unification\-Scopes v} and
...
\begin{itemize}
  ...
  have the scope as reported by \tc{m}. This is our main scope invariant for this
  section.
\end{itemize}
...
This version of \tc{skolemize} cannot be checked with Liquid Haskell because of
transient limitations of the tooling, so we assume the refinement type signature
by resorting to the principle of only checking what is cost-effective. The rest
of the code doesn't benefit less because of it.
...
The predicate \tc{consistentScopesTerm} is only used in refinement types, and
checks that the scopes of the unification variables in a term have the scopes
as defined by the given map.
...
The interesting case is the one of skolem function applications, where the
predicate only holds if the domain of the suspended substitution agrees
with the finite map \tc{m}. Crucially, this ensures that different
occurrences of the same unification variable cannot have different scopes.
...
with these parameters. They do not play any role in the behavior of the
functions, but they simplify a great deal expressing the consistency of
scopes, which constitute our next principle.

\begin{principle}
  In order to streamline the expression of the properties to check, identify
  any implicit knowledge that is missing from the inputs of a function, and
  consider making this knowledge explicit by enriching the input.
\end{principle}
\unsureF{Is this insightful enough? You mention the rapier might have been inspired by something like this}
...
annotated to help the Liquid Haskell check the refined signature. We only needed
a few lemmas for this study. Here's one of them. It establishes an
expectation on the meaning of the \tc{consistentScopesSubst} predicate.
...
@-}
lemmaConsistentScopesSubst :: IntMap (Set Int) -> Subst Term -> ()
lemmaConsistentScopesSubst _ _ = ()
\end{verbatim}

This lemma could be proved if we committed to a particular representation of the
\tc{Subst} type. But here we leave it as a requirement for the integration of
implementations of \tc{Subst}.\unsureF{Can we have a principle to use assumptions to define integration properties?}

Lemmas are ordinary functions, Liquid Haskell doesn't automatically make use of
them, but it comes with an idiom. When you want to ask Liquid
Haskell to use a lemma when checking the expected refinement type of an expression
\tc{e}, you are to invoke the lemma as
\begin{verbatim}
e ? lemmaConsistentScopesSubst
\end{verbatim}
The changes to \tc{unifyEq} are annotations of this form, where \tc{?} has no
effect when executing the program. In the case of the \tc{unifyEq} function,
it would be inserted as follows.

\begin{verbatim}
 unifyEq t0 t1@(SA (i, s))
   | Just s' <- inverseSubst $ narrowForInvertibility (freeVars t0) s
-  , let t' = substitute s' t0
+  , let t' = substitute (s' ? lemmaConsistentScopesSubst m s') t0
   , not (Set.member i (skolemSet t'))
   , Set.isSubsetOf (freeVars t') (domain s)
   = Just [(i, t')]
\end{verbatim}

For a lemma that we do need to prove, we can look at \tc{lemma\-Consistent\-Superset}.
This lemma checks our intuition that if a formula has consistent scopes for
a given finite map \tc{m0}, it also should be consistent with any finite map
that is a superset of \tc{m0}.
...
The function \tc{consistentScopes} does for formulas what \tc{consistentScopesTerm}
does for terms. The proof of this lemma is a regular and rather straightforward
recursive function.
...
The proof continues in lemma \tc{lemmaConsistentSupersetTerm} in similar fashion
until it reaches \tc{lemmaConsistentScopesSubst}, another fringe lemma about
the meaning of scope consistency of substitutions that becomes an integration requirement.
...
The lemma functions are so terse because Liquid Haskell is doing the rote folding
and unfolding of the \tc{consistentScopes} predicate for every case of formula and
term, plus the reasoning with the subset relationship of maps. Lemmas like these
do not only help to check the subject program, but also help keeping in check the
definition of \tc{consistentScopes}, should it be updated in the
future.\unsureF{Writing these proofs with AI code autocompletion is a breeze. But
...
convoluted. Thus we arrive at the following principle.

\begin{principle}
  SMT solvers are more effective when they can leverage their theories.
  Prefer writing refinement types by appealing to theories known by your system.
\end{principle}\unsureF{Arnaud, please check the appropriateness of the introduction of this principle here}.
...
publications/ifl25-rtdd/examples/Unif2.hs:580:18: error:
    ...
    in the context
      ?g : {?g : Maybe (Subst Term) |
               ?g == Just ss'
            && ?g == inverseSubst s m (narrowForInvertibility (freeVars t1) ss)}
...
study. Our source code contains comments explaining the bugs where we were
