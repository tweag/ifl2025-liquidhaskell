...
modify the Liquid Haskell implementation to support it, modifications which
constitute our third contribution. Lastly, we offer an informed discussion
...
is why our achievement is partial. In Section~\ref{limitations-of-liquid-haskell}
we discuss the unimplemented parts.
...
scopesTerm (SA (i, s)) = IntMap.singleton i (domain s)
...
in the refinement type of the input. The second conjunct of the refinement type
...
automatic except for a lemma that relates the function \tc{fromListSubst} with
the skolems present in the terms of the resulting substitution.
...
Here the function \tc{scopesSubst} provides the skolems present in the range of
the substitution. This lemma and function \tc{scopesSubst} depend on the representation
of substitutions and therefore are assumed to work as the refinement type
signature describes for the sake of this study. The lemma is used to relate
the skolems in the substitution \tc{unifsT1Subst} in the implication equation of
\tc{unify} with the skolems of the input formula.
...
\item test in an example that consistentSkolemScopes works as we expect
...
The checking of unification is a plausible future, though one that requires a
modest enhancement to Liquid Haskell. At the moment Liquid Haskell does not
allow to implement maps.
