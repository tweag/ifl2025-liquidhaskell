...
to apply substitutions in formulas and terms respectively, \tc{substituteSkolems} to  substitute
...
case aren't essential to our the discussion in this article.
...
Liquid Haskell helpfully lets us treat the state monad has being equipped with
...
\tc{existsCount v = 0}, the rest are invariant used by the recursive calls of
\tc{skolemize}.
\begin{itemize}
  \item \tc{existsCount v = 0} means that \tc{skolemize} returns a formula without
  existential quantifier. As is a requirement of \tc{unify}.
  ...
  has the same scope. This is our main scope invariant for this section.
\end{itemize}
...
unionCommutes (scopesTerm t0) (scopesTerm t1)
\end{verbatim}
The \tc{unionCommutes m1 m2} predicates states that \tc{union m1 m2 = union m2
...
opportunity to talk about totality requirement. Here is its signature.
...
loop and any attempt at using \tc{True = False} would diverge. But \tc{bad} is
...
Haskell need a little help in the form of a termination metric (here the number
...
% LocalWords:  equalities GADTs injective skolem postconditon
