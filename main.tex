...
a name \tc{b} such that addint \tc{b} to scope set \tc{n} results in the scope
set \tc{l}.
...
It make the type \tc{Scope} abstract to enforce that binders are always
...
Another way, besides adding computationally irrelevant arguments, like in the
previous sections for the refined implementation to differ from its classical
version is with the addition of lemmas.
...
theory of finite map (see Section~\ref{extending-liquid-haskell}) is crucial in
making this proof so terse.

Ultimately, the proof of \tc{lemmaConsistentSupersetTerm} depends on the
following lemma which we must assume of the substitution data type
(unsurprisingly, the substitution interface needs to satisfy more properties
than in Section~\ref{the-rapier-with-refinement-types} to accommodate the
unification variables' scopes)
