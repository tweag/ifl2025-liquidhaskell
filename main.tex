...
solver, to support our case study.\improvement{It would be better to have a
  reference to a part of the paper for the implementation}
...
\begin{verbatim}
  | member i scope ->
      Lam i $ substitute (insert i scope) (extendSubst s i (Var i)) e
\end{verbatim}
and it will still typecheck\unsure{Is this true? \@Facundo: please check}. So that
(in pseudo-Haskell)
...
as in Section~\ref{the-rapier-with-stronger-types}.\unsure{Maybe we need to
  redesign this code snippet for an abstract member function. We need to discuss
  this}
...
refreshed when traversed. The full code for this example can be found in the
...
\sourcefile{Subst2.hs}\critical{Make sure that this is the right file name in
  when submitting}
