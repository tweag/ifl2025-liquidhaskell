...
; Similar to: empty == zipWith difference_strict_p2p x y
...
\subsection{SMT solvers for programming}
...
assume that \tc{lemmaFreeVarsSubstUnionGoal} stands for the returned
refinement type of lemma \tc{lemmaFreeVarsSubstUnion}.

\begin{verbatim}
(assert (forall ((s (Subst Term)) (t Term)) (lemmaFreeVarsSubstUnionGoal s t)))
\end{verbatim}

When trying to see whether other assertions can be satisfied, the SMT solver
...
is that one needs to narrow the supported properties to a feasible set that allows
to write a library. If we wanted to add static checks like those of the unification
example, it is unlikely that we would come across such a dedicated library, if one
wasn't written to reason about skolems and their domains.


Compare to the foil once more.
