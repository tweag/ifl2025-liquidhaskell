...
Liquid Haskell is a
plugin for Haskell which statically check that programs respect signatures
...
Liquid Haskell doesn't have a good built-in understanding of the \tc{maxElement}
...
\begin{principle}
  Refinement type permit much smaller trusted code base than
...
\begin{verbatim}
s :: Substitution (ScopedExp scope)
\end{verbatim}
also has (refined) type
\begin{verbatim}
s :: Substitution (ScopedExp (insert i scope))
\end{verbatim}
...
We expect the set \tc{sf} to be a subset of \tc{se}. This is two reflect the
...
to hang at the moment.\unsureF{Such a terrible ending for a great section. I think I'll try to determine who's hanging at least}
...
Section~\ref{ensuring-the-scope-set-is-checked}.\change{We need to fish for a different lemma now that this section is removed.}
