...
a compiler, complements an ordinary type checker and can, in fact, be used much
the same way. SMT solvers and type checker are good at enforcing different kinds of
...
\begin{principle}
  In general, refinement types allow to reduce the trusted code base, but they also offer
    ...
    hand than with the SMT solver, you should assume the property.
\end{principle}

Note that \tc{assume} is safe, in the sense that if a program assumes an
incorrect assumption, it will not yield to any memory corruption. The program
will still run, it may just return incorrect or unexpected values. In that
sense, \tc{assume} is akin to adding a function to an abstract type: it's type
safe, even though the type-checker cannot ensures that it preserves the abstract
type's invariants. This is a way in which refinement types differ from types
greatly: bypassing the type-checker and assuming the wrong type (\emph{e.g.}
with \tc{unsafePerformIO} in Haskell) will cause memory corruptions.
...
our study we take an abstract type and \tc{assume} the necessary property that a
substitution type need to verify in order to qualify. Even though we used a
concrete type for the sake of testing (remember, this isn't a verification
project, it's ordinary programming and it needs testing), representing the type
like that ensures that we can support any efficient substitution type.
...
We pass the map \tc{m} as a monadic state, because we don't
...
in keeping with the principle of only checking what is cost-effective. The rest
...
The predicate \tc{consistentScopesTerm s m} is only used in refinement types, and
checks that the domains of the unification variables' substitutions in a term
\tc{s} are the scopes given by \tc{m}.
...
functions' preconditions, but it's more involved, and requires a great deal more
lemmas to convince the SMT solver.

\begin{principle}
  It's easier both for an SMT solver and to express properties to begin with to
  use explicit assumptions rather than reconstructing implicit assumptions.
  Don't hesitate to pass assumptions as arguments to functions, even if those
  arguments aren't used by the function.
\end{principle}

Note that compilers typically remove such obviously unused arguments during
compilation. GHC certainly does. So there's essentially no computational cost to
...
Lemmas aren't used automatically, this is how Liquid
Haskell is instructed to use them.
...
completion. Since lemmas don't have computational content (\tc{\{p\}} is a
...
A type-checker approach, however, is likely to produce error messages that
